...
class QLearning(object):
    def __init__(self):
        ...
        self.q_matrix_pub = rospy.Publisher(
            "/q_learning/q_matrix", QMatrix, queue_size=1)
        self.robot_action_pub = rospy.Publisher(
            "/q_learning/robot_action", RobotMoveDBToBlock, queue_size=1)
        self.is_converged_pub = rospy.Publisher(
            "/q_learning/q_matrix_converged", Bool, queue_size=1)
        ...
        # The time the last reward was received. Used to catch on bogus reward 
        # values from the world resetting
        self.time_last_reward = 0

        # Time delay between receiving a reward and acting on it. Should have a 
        # non-zero value for performance issues
        self.delay = 0.1

        # Used to keep track of the last n q_matrix update values
        # The algorithm will determine if it's converged if the last n values 
        # did not produce a new update
    ...
    def sample_action(self, init=False):
        """Samples a valid action based on the current state and publishes it

        Args:
            init (bool, optional): True if sampling from initial state
        """        
    ...
    def reward_callback(self, data: QLearningReward):
        """Callback for when a reward is received

        Args:
            data (QLearningReward): Reward data
        """        
        # Catching bogus reward where reset_world.py sends us a reward when the 
        # world resets (this should not always happen but we need to catch it 
        # for our code's logic)
        ...
        if (action_taken == -2):  
            # No valid action, i.e. all 3 dumbbells already in front of 3 
            # distinct blocks
            return

        current_state = self.state
        next_state = np.where(
            np.array(self.A[current_state]) == action_taken)[0][0]
        alpha = 1
        gamma = 0.2

        # Applying basic q-learning update formula
        Q_st_at = self.Q.q_matrix[current_state][action_taken]
        updated_q_matrix_val = Q_st_at + alpha * \
            (data.reward + gamma * max(self.Q.q_matrix[next_state]) - Q_st_at)
        ...
        self.running_update_window.append(
            str(Q_st_at) + "->" + str(updated_q_matrix_val))
...
def count_moves_from_states(state1, state2):
    ...
    Returns:
        (int, array): (number of moves needed to reach next state, list of 
        moves made to transition to next state given as (dumbbell, location))
    """
    num_moves = 0
    # Array of tuples e.g. (RED, 1) - (dumbbell color, move location)
    moves = []
